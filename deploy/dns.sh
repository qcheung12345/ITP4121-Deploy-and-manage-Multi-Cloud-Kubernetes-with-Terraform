...
AWS_HOST=$(echo "$ENDPOINTS"   | python3 -c "import sys,json; print(json.load(sys.stdin)['aws'])")
...
echo ""
printf "  🟧  AWS   (EKS, us-east-1)          → %s\n" "$AWS_IPS"
printf "                                          ELB: %s\n" "$AWS_HOST"
printf "  🟦  Azure (AKS, southafricanorth)   → %s\n" "$AZURE_IP"
printf "  🟩  GCP   (GKE, asia-east2)         → %s\n" "$GCP_IP"
echo ""
echo "────────────────────────────────────────────────────────────────"
echo "Querying Route53 20 times (weighted routing: AWS 33 / Azure 33 / GCP 34)"
echo "────────────────────────────────────────────────────────────────"
echo ""
...
for i in $(seq 1 20); do
    ...
    if echo " $AWS_IPS " | grep -q " $IP "; then
        LABEL="🟧 AWS  "
        AWS_COUNT=$((AWS_COUNT+1))
    elif [ "$IP" = "$AZURE_IP" ]; then
        LABEL="🟦 Azure"
